...
function AdatokAtalakitasa(adatHalmaz) {
    let beolvasottAdatok = [];

    for (let i = 0; i < adatHalmaz.length; i++) {
        let daraboltAdatok = adatHalmaz[i].split(" ");
        let ujAdat = {
            helyezes: Number(daraboltAdatok[0]),
            csapatMeret: Number(daraboltAdatok[1]),
...
async function AdatokFeltoltese() {
    try{
        ...
        const eredmeny = await collection.insertMany(AdatokAtalakitasa(eredmenyek));
        const muveletek = eredmeny.insertedCount;


        console.log(muveletek, " adat kerult feltoltesre.");
        client.close();
    }
    catch (err) {
